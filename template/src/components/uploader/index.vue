 ...
 * @Last Modified time: 2018-01-21 18:35:07
 * @DESC:  上传组件
 */
...
export default {
  ...
  props: {
    ...
    start: {
      // 当非自动上传的时候，如果此值传true，开始上传文件 (auto为false时)
      type: Boolean,
      default: false,
    },
  },
  ...
  methods: {
    ...
    upload(options) {
      let self = this;
      const {
        url, file, fileVal, xhrFields, params,
      } = options;
    ...
    noop() {},
    createRandom() {
      return parseInt(Math.random() * 10e8, 10) + new Date().getTime();
    },
  },
  mounted() {
